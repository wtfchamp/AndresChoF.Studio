...
;(function(window) {
	 ...
	 * Circle Slideshow
	 */
	function CircleSlideshowF(el, options) {
		this.el = el;
		this.options = extend( {}, this.options );
		extend( this.options, options );

		// items
		this.items = [].slice.call(this.el.querySelectorAll('.slideF'));
		// total items
		...
		this.isClosed = true;
		this.elementos = document.querySelectorAll('.st1F');
		this._init();
	}

	//CircleSlideshowF.prototype.options = {};

	CircleSlideshowF.prototype._init = function() {
		...
		this.navLeftCtrl.className = 'navbuttonF navbutton--nextF';
		this.navLeftCtrl.setAttribute('aria-label', 'Next item');
		this.navLeftCtrl.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="100px" height="30px" viewBox="0 0 35.2 70.4"><polyline fill="#CAF903" class="navbutton__lineF" points="0,70.4 35.2,35.2 0,0"/></svg>';

		this.navRightCtrl = document.createElement('button');
		this.navRightCtrl.className = 'navbuttonF navbutton--prevF';
		this.navRightCtrl.setAttribute('aria-label', 'Previous item');
		this.navRightCtrl.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="100px" height="30px" viewBox="0 0 35.2 70.4"><polyline fill="#CAF903" class="navbutton__lineF" points="35.2,70.4 0,35.2 35.2,0"/></svg>';
		...
		this.items.forEach(function(item) {
			var slideEl = item.querySelector('.slide__itemF');
		});

		// position current item:
		classie.add(this.items[this.current], 'slide--currentF');
		classie.add(this.elementos[this.current], 'st0F');
		classie.remove(this.elementos[this.current], 'st1F');

		// event binding
		this._initEvents();
	};

	CircleSlideshowF.prototype._initEvents = function() {
		var self = this;
	...
	CircleSlideshowF.prototype._navigate = function(dir) {
		if( this.isExpanded ) {
			return false;
		}
		...
		var self = this,
			...
			currentEl = itemCurrent.querySelector('.slide__itemF');
			//
		...
		if( dir === 'right' ) {
			...
			classie.add(this.elementos[this.current], 'st0F');
			classie.remove(this.elementos[this.current], 'st1F');
			classie.remove(this.elementos[this.current > 0 ? this.current - 1 : this.itemsTotal-1], 'st0F');
			classie.add(this.elementos[this.current > 0 ? this.current - 1 : this.itemsTotal-1], 'st1F');
			...
			classie.add(this.elementos[this.current], 'st0F');
			classie.remove(this.elementos[this.current], 'st1F');
			classie.remove(this.elementos[this.current < this.itemsTotal-1 ? this.current + 1 : 0], 'st0F');
			classie.add(this.elementos[this.current < this.itemsTotal-1 ? this.current + 1 : 0], 'st1F');
		}

		var itemNext = this.items[this.current],

			nextEl = itemNext.querySelector('.slide__itemF');
		...
		dynamics.animate(nextEl,
			...
			},
			{
				...
				complete: function() {
					self.items.forEach(function(item) { classie.remove(item, 'slide--currentF'); });
					classie.add(itemNext, 'slide--currentF');
				}
			}
		);
	...
	window.CircleSlideshowF = CircleSlideshowF;
